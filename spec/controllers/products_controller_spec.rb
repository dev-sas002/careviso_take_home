...
RSpec.describe ProductsController, type: :controller do
  ...
  describe "DELETE #destroy" do
    ...
    it "deletes a product that belongs to a package" do
      create(:package, products: [product])

      expect do
        delete :destroy, params: { id: product.id }
      end.to change(Product, :count).by(-1).and change(PackageProduct, :count).by(-1)
    end

    it "deletes a product that belongs to an order" do
      create(:order, products: [product])

      expect do
        delete :destroy, params: { id: product.id }
      end.to change(Product, :count).by(-1).and change(OrderProduct, :count).by(-1)
    end
  end

  describe "a missing product" do
  ...
  describe "pagination" do
    it "caps the page size and ignores nonsense page numbers" do
      create_list(:product, 5)
      stub_const("Paginatable::DEFAULT_PER_PAGE", 2)

      get :index, params: { page: "not-a-number" }

      expect(assigns(:products).size).to eq(2)
      expect(assigns(:pagination).page).to eq(1)
      expect(assigns(:pagination).total_pages).to eq(3)
    end
  end
end
